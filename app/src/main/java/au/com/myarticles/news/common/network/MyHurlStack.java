...
/**
 * HurlStack implementation to support reading multi value headers.
 * <p/>
 */
    ...
    for (Entry<String, List<String>> header : connection.getHeaderFields().entrySet()) {
      if (header.getKey() != null) {
        for (int i = 0; i < header.getValue().size(); i++) {
          Header h;
          if (i == 0) {
            h = new BasicHeader(header.getKey(), header.getValue().get(i));
          } else {
            h = new BasicHeader(header.getKey() + "-Extra-" + i, header.getValue().get(i));
          }
